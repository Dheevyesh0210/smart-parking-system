...
>>>>>>> 646cb52bdb5ca35d795aa6cb235cc59d7c387d2f

ADMIN_CREDENTIALS = {"admin": "admin1234", "operator": "operator123"}
...
def checkout_booking_in_db(booking_id):
    ...
    try:
        ...
        cur.execute('UPDATE bookings SET status = %s, checkout_time = CURRENT_TIMESTAMP WHERE id = %s',
                    ('completed', booking_id))
        cur.execute('UPDATE parking_slots SET is_reserved = FALSE, updated_at = CURRENT_TIMESTAMP WHERE slot_id = %s',
                    (booking['slot_id'],))
        conn.commit()
        cur.close()
        conn.close()
        return {'id': booking['id'], 'name': booking['name'], 'slot': booking['slot_id'], 'status': 'completed'}
    except Exception as e:
...
BUTTON_STYLE = {
    'backgroundColor': ACCENT_COLOR, 'color': 'white', 'border': 'none',
            ...
            html.Div([
                dcc.Input(id='username-input', type='text', placeholder='Username',
                          style={'width': '100%', 'padding': '12px', 'marginBottom': '15px',
                                 ...
                                 'backgroundColor': CARD_BG, 'color': TEXT_PRIMARY}),
            ...
            html.Div([
                ...
                dcc.Input(id='booking-license', type='text', placeholder='License Plate (e.g., MU-1234)',
                          style={'width': '100%', 'padding': '12px', 'marginBottom': '10px',
                                 'borderRadius': '6px', 'border': '1px solid #334155',
                                 'backgroundColor': CARD_BG, 'color': TEXT_PRIMARY}),
                dcc.Input(id='booking-duration', type='number', placeholder='Duration (hours)',
...
def render_dashboard_content(df, stats):
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.P("Avg Duration", style={'color': TEXT_SECONDARY, 'margin': 0, 'fontSize': '12px'}),
                    html.H4(f"{stats['avg_duration']:.1f}h", style={'color': TEXT_PRIMARY, 'margin': 0})
                ], style={'backgroundColor': CARD_BG, 'padding': '15px', 'borderRadius': '8px', 'flex': 1,
                          'marginRight': '10px'})
            ], style={'flex': 1}),

            html.Div([
                html.Div([
                    html.P("Overstays", style={'color': TEXT_SECONDARY, 'margin': 0, 'fontSize': '12px'}),
                    html.H4(f"{stats['overstay_count']}",
                            style={'color': DANGER_COLOR if stats['overstay_count'] > 0 else TEXT_PRIMARY, 'margin': 0})
                ], style={'backgroundColor': CARD_BG, 'padding': '15px', 'borderRadius': '8px', 'flex': 1,
                          'marginRight': '10px'})
            ], style={'flex': 1}),
            ...
            html.Div([
                html.Div([
                    html.P("Reserved", style={'color': TEXT_SECONDARY, 'margin': 0, 'fontSize': '12px'}),
                    html.H4(f"{stats['reserved']}", style={'color': INFO_COLOR, 'margin': 0})
                ], style={'backgroundColor': CARD_BG, 'padding': '15px', 'borderRadius': '8px', 'flex': 1,
                          'marginRight': '10px'})
            ], style={'flex': 1}),
            ...
            html.Div([
                html.Div([
                    html.P("Maintenance", style={'color': TEXT_SECONDARY, 'margin': 0, 'fontSize': '12px'}),
                    html.H4(f"{stats['maintenance']}", style={'color': WARNING_COLOR, 'margin': 0})
                ], style={'backgroundColor': CARD_BG, 'padding': '15px', 'borderRadius': '8px', 'flex': 1})
            ], style={'flex': 1})
        ], style={'display': 'flex', 'marginBottom': '20px'}),
        ...
        html.Div([
            html.Div([
                ...
                dcc.Graph(
                    figure=go.Figure(
                        data=[go.Scatter(x=list(timestamp_history), y=list(occupancy_history),
                                         mode='lines+markers', line=dict(color=ACCENT_COLOR, width=2),
                                         marker=dict(size=6))],
                        layout=go.Layout(
                            plot_bgcolor=CARD_BG, paper_bgcolor=CARD_BG,
                            font=dict(color=TEXT_PRIMARY), height=250,
            ...
            html.Div([
                ...
                dcc.Graph(
                    figure=go.Figure(
                        data=[go.Bar(x=list(timestamp_history), y=list(revenue_history),
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    ...
                    html.Div(style={
                        'width': '100%', 'height': '8px', 'backgroundColor': '#334155',
                        'borderRadius': '4px', 'marginTop': '8px', 'overflow': 'hidden'
                    }, children=[
                        html.Div(style={
                            'width': f"{z['percentage']}%", 'height': '100%',
                            'backgroundColor': SUCCESS_COLOR if z['percentage'] > 50 else WARNING_COLOR if z[
                                                                                                               'percentage'] > 20 else DANGER_COLOR
                        })
                    ])
        ...
        html.Div([
            ...
            dash_table.DataTable(
                ...
                columns=[{"name": i, "id": i} for i in ['Slot ID', 'Zone', 'Status', 'Vehicle',
                                                        'License', 'Duration', 'Total']],
                style_table={'overflowX': 'auto'},
                style_cell={'backgroundColor': CARD_BG, 'color': TEXT_PRIMARY, 'textAlign': 'left',
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.P("Total Bookings Value", style={'color': TEXT_SECONDARY, 'margin': 0})
            ], style=METRIC_CARD)
        ...
        html.Div([
            html.Div([
                html.Div([
                    ...
                    html.Div([
                        html.Span(
                            "✅ ACTIVE" if b['status'] == 'active' else "✓ COMPLETED",
                            style={
                ...
                html.Div([
                    ...
                    html.Span(f"👤 {log['user_name']}",
                              style={'color': ACCENT_COLOR, 'fontSize': '14px', 'width': '120px',
                                     'marginLeft': '15px'}),
                    html.Span(f"📋 {log['action']}",
                              style={'color': TEXT_PRIMARY, 'fontSize': '14px', 'width': '150px',
                                     'marginLeft': '15px'}),
                    html.Span(log['details'],
                          ...
                          'backgroundColor': CARD_BG, 'borderRadius': '8px', 'marginBottom': '8px'})
            ]) for log in logs
        ])
    ])
...
def update_admin_content(n, view, session_data):
    ...
    if df.empty:
        return html.Div([
            html.H2("⚠️ Database Connection Error", style={'color': WARNING_COLOR, 'textAlign': 'center'}),
            html.P(f"DATABASE_URL: {DATABASE_URL}", style={'color': TEXT_SECONDARY, 'fontSize': '12px'})
...
def submit_booking(n_clicks, name, phone, vehicle, license_plate, duration):
    if n_clicks > 0:
        if not all([name, phone, vehicle, license_plate, duration]):
            return html.Div("❌ Please fill all fields", style={'color': DANGER_COLOR, 'padding': '10px',
        ...
        if len(available_df) == 0:
            return html.Div("❌ No slots available", style={'color': DANGER_COLOR, 'padding': '10px',
                                                           'backgroundColor': CARD_BG, 'borderRadius': '6px'})
        ...
        booking = {
            'id': booking_id, 'name': name, 'phone': phone, 'vehicle': vehicle,
